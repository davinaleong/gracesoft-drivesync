...
- [ ] Reference adapters' upsert/delete/query round-trip against real staging infra: Pinecone for `VectorStore`, OpenAI for `EmbeddingProvider` (beyond the mocked contract suite — real API behavior, quotas, latency). OpenAI half done (M7, live embedding call verified). Pinecone half explicitly not done — the user stopped the tool call that would have created a real throwaway index (M8).
