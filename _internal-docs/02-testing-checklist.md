...
- [x] Content hashing: identical content → identical hash; modified content → different hash. `src/dedup/contentHash.test.ts` (M10) — missed when M10's checklist entries were written; caught during M17's review pass.
...
- [x] Dedup logic: unchanged files skipped on re-sync, only diffs re-embedded. `src/dedup/contentHash.test.ts` — identical/differing/whitespace-sensitive content hashing, and the changed/unchanged decision including the "no previous hash" always-changed case. Not yet wired into a real sync run (no sync job exists until M12).
- [x] Vector ID scheme: stable, collision-free vector IDs (`{fileId}-{chunkIndex}`); upserts don't duplicate — provider-agnostic, covered as part of the `VectorStore` contract suite. `src/dedup/vectorIds.test.ts` — `buildVectorId` stays unambiguous even when `fileId` itself contains hyphens; `computeStaleVectorIds` covers unchanged/grown/shrunk/fully-deleted/brand-new chunk-count transitions.
...
- [x] **Namespace isolation**: account A's retrieval query never returns account B's chunks, even under a shared index/collection — test with two real accounts and overlapping/similar content. Runs against whichever `VectorStore` adapter is active (part of the contract suite above), not just Pinecone. Covered by `vectorStore.contract.ts`'s "keeps namespaces isolated" test (real cosine-similarity fake, overlapping vector ID/content across two namespaces) — proves the actual isolation mechanism (namespace = accountId, never anything else) rather than two real `Account` rows specifically, since `RetrievalService` has no code path that could bypass this.
...
- [x] Reference adapters' upsert/delete/query round-trip against real staging infra: Pinecone for `VectorStore`, OpenAI for `EmbeddingProvider` (beyond the mocked contract suite — real API behavior, quotas, latency). OpenAI verified live in M7. Pinecone verified live while troubleshooting the `OPENAI_EMBEDDING_DIMENSIONS` fix (post-M18): real embed → real upsert into the user's actual `gracesoft-drivesync-dev` index → real query returned the upserted vector → real delete cleaned it up.
- [x] **Provider swap**: switching `EmbeddingProvider`/`VectorStore` config on a deployment with existing data behaves as documented (fails clearly or triggers full resync — not silent corruption or mixed-dimension writes). `src/vectorstore/assertProviderCompatibility.test.ts` — mismatched dimensions throw a clear, actionable error; matching dimensions and a brand-new (no-dimension-yet) index both pass. Verified with a fake `EmbeddingProvider`/`VectorStore`, not yet wired into a real sync run (no sync job exists until M12) or exercised against a real Pinecone index.
- [x] Prisma migrations apply cleanly to a fresh DB; schema matches the multi-tenant model (accounts, API keys, DriveFolder, DriveFile all correctly related). Verified locally against a real throwaway Postgres: all three migrations (`init`, `add_drive_folder`, `add_drive_file`) apply in sequence to a fresh database. CI independently re-verifies via `prisma migrate deploy` on push.
- [x] Retrieval endpoint returns correct top-k chunks **with chunk text included** (v2 change from v1's metadata-only response), plus accurate source attribution. `src/retrieval/retrievalService.test.ts` + `src/retrieval/retrievalRouter.test.ts` — text/score/fileId/fileName/section returned correctly, namespace scoped to the caller's own accountId, topK validated (missing query, topK over the max both rejected with 400), API-key-gated. Not yet verified against a real synced folder end-to-end.
- [x] MCP server: tool calls return correctly scoped, correctly shaped results against a real connected folder; an MCP client with an invalid/revoked key is rejected the same way the REST API would reject it. `src/mcp/mcpServer.test.ts` (real MCP `Client`/`Server` over `InMemoryTransport`) + `src/mcp/mcpRouter.test.ts` (real streamable HTTP over a real listening server, including the 401-without-auth-header case reusing `requireApiKey`). Not yet verified against a real connected folder end-to-end.
## Failure & resilience tests
 
- [x] Drive API rate limit/429 → retry with backoff, doesn't crash the sync job (a known, carried-forward gap from v1 — worth closing this time). Closed in M17: `src/drive/driveClient.ts`'s `withRetry` wraps all four real Drive API calls (retries 429/5xx with exponential backoff, passes 404/403 straight through unretried). `src/drive/withRetry.test.ts` — 6 tests covering retry-then-succeed, 5xx, non-retryable passthrough, retry exhaustion, and backoff timing.
- [x] Embedding API timeout/failure → job fails gracefully, no partial/corrupt state, retriable on next run. `src/sync/syncFolder.test.ts` ("records an unexpected error without throwing") — a throwing embedding/vector-store call is caught per file, recorded in `failedFiles`, and never reaches `fileRepository.upsertSynced` for that file, so no partial state is written; the next run re-attempts it naturally since its `FileRepository` row wasn't updated.
- [x] Vector store write failure mid-batch → no silent data loss; sync marked failed/partial, not falsely "success". Same test as above — a failing `vectorStore.upsert` is recorded as a per-file failure, never silently counted as a success.
- [x] Malformed/corrupted file in a folder → skipped with a logged warning, doesn't halt the rest of that account's sync or any other account's. Per-file skip-and-continue was already covered (`syncFolder.test.ts`); closed the missing half in M17 — `worker.ts` previously only logged folder-level failures, never the per-file failures embedded in an otherwise-*successful* folder outcome. Now logs `logger.warn` with every skipped file across the run. Verified by code review/typecheck, not yet exercised against a real sync run with a real malformed file. **Post-M18**: found live during a local test run that no per-file status was logged at all (only folder/run-level aggregates) — `syncFolder.ts` now logs `file added`/`file updated`/`file deleted`/`file sync failed` immediately as each outcome is decided, and `syncAllFolders.ts` logs a per-folder `folder sync completed` breakdown; see `_internal-docs/progress/M15-observability-addendum-file-level-logs.md`.
- [ ] Concurrent sync runs (scheduled job overlaps a manual trigger, or two accounts sync simultaneously) → locking prevents duplicate/conflicting writes without serializing unrelated accounts unnecessarily. Partially true by construction, not fully closed: BullMQ's default `Worker` concurrency (1) means one process never runs two jobs at once, and BullMQ's own per-job lock stops the *same* job being double-processed across multiple worker processes; per-account rate limiting (M12) already lets different accounts run fully in parallel. What's **not** built: a cross-process lock preventing two *different* jobs (e.g. a scheduled tick and a manual trigger, each running on a different worker process) from both calling `runSync()` and processing the same folder concurrently. Flagged as a real gap rather than silently assumed safe — worth a Redis-based per-folder lock as a fast-follow if horizontal worker scaling becomes a real deployment pattern.
...
- [x] **API auth survives a database outage**: a `requireApiKey` lookup failure (DB unreachable, connection error) returns a clean `500`, not an unhandled rejection that crashes the whole process. Found live while smoke-testing M14 — Express 4 doesn't await async middleware, so the original code's uncaught rejection took down the entire server on any DB hiccup during auth, REST or MCP. Fixed via `next(err)`; regression test in `src/middleware/requireApiKey.test.ts`; re-verified live (real server survived a real DB-unreachable error and correctly served the next request).
## Scheduling & observability
 
- [x] Cron/queue trigger fires reliably on schedule. Verified live in M17: ran the real worker with `SYNC_CRON="*/1 * * * *"` against real Redis and watched it fire completely unattended at 7 consecutive exact 60-second boundaries — no manual triggering, proving the repeatable job actually fires on its own, not just when poked.
